              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="range-wrapper">
                  <input
                    ...
                    min="10"
                    :max="Math.min(autoGeneratedRounds, 40)"
                    step="5"
                    class="minimal-slider"
                    :style="{ ‘--percent’: ((customMaxRounds - 10) / (Math.min(autoGeneratedRounds, 40) - 10)) * 100 + ‘%’ }"
                  />
                  <div class="range-marks">
                    ...
                    <span
                      ...
                      :style="{ position: ‘absolute’, left: `calc(${(40 - 10) / (Math.min(autoGeneratedRounds, 40) - 10) * 100}% - 30px)` }"
                    >40 (recommended)</span>
                    <span>{{ Math.min(autoGeneratedRounds, 40) }}</span>
                  </div>
                </div>
                <p class="rounds-hint">We recommend 20–40 rounds for a first simulation. More rounds = more detailed results but higher API cost.</p>
              </div>

              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  <div class="auto-value">
                    <span class="val-num">{{ Math.min(autoGeneratedRounds, 40) }}</span>
                    <span class="val-unit">rounds</span>
                  </div>
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        At 100 agents: approx. {{ Math.round(Math.min(autoGeneratedRounds, 40) * 0.6) }} min runtime
                      </span>
                    </div>
                    <div class="auto-desc">
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    params.maxRounds = Math.min(customMaxRounds.value, 40)
    addLog(`Starting simulation with ${params.maxRounds} rounds (custom)`)
  } else {
    const cappedRounds = Math.min(autoGeneratedRounds.value, 40)
    params.maxRounds = cappedRounds
    addLog(`Starting simulation with ${cappedRounds} rounds (auto)`)
  }
