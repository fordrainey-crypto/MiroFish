    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create a new simulation instance and fetch the simulation world parameter template.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Automatically extracts entities and relationships from the knowledge graph, initializes simulation agents, and assigns each unique behaviors and memories based on real-world seeds.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Seed-Associated Topics</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM intelligently sets world time flow, recommendation algorithms, active hours per agent, post frequency, event triggers, and more based on the simulation requirements.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Recommendation Algorithm</span>
              </div>
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  ...
                  <div class="platform-params">
                    ...
                    <div class="param-row">
                      <span class="param-label">Popularity</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generates initial activation events and trending topics based on narrative direction to seed the simulation world's starting state.
          </p>
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div class="topics-section">
              <span class="box-label">Trending Topics</span>
            ...
            <div class="initial-posts-section">
              <span class="box-label">Activation Sequence ({{ simulationConfig.event_config.initial_posts.length }})</span>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish will simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real time. Each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes passing.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>At 100 agents: approx. {{ Math.round(customMaxRounds * 0.6) }} min runtime</span>
                  </div>
                </div>
                ...
                <div class="range-wrapper">
                  <input
                    ...
                    :style="{ '--percent': ((customMaxRounds - 10) / (sliderMax - 10)) * 100 + '%' }"
                  />
                  <div class="range-marks">
                    <span>10</span>
                    <span
                      class="mark-recommend"
                      :class="{ active: customMaxRounds === autoGeneratedRounds }"
                      @click="customMaxRounds = autoGeneratedRounds"
                      :style="{ position: 'absolute', left: `calc(${(autoGeneratedRounds - 10) / (sliderMax - 10) * 100}% - 30px)` }"
                    >{{ autoGeneratedRounds }} (AI suggested)</span>
                    <span>{{ sliderMax }}</span>
                  </div>
                </div>
                <p class="rounds-hint">AI-suggested rounds are based on your scenario's time span. More rounds = more detailed results but higher API cost.</p>
              </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        At 100 agents: approx. {{ Math.round(autoGeneratedRounds * 0.6) }} min runtime
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">First time? We recommend switching to Custom mode to reduce rounds for a faster preview. ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
// Slider max is 2× the auto-generated recommendation, snapped to step=5
const sliderMax = computed(() => {
  if (!autoGeneratedRounds.value) return 100
  return Math.round(autoGeneratedRounds.value * 2 / 5) * 5
})

// When config first loads, initialize slider to the AI-suggested value so the
// user can see the full range and isn't stuck looking at a thumb stuck at 40
watch(autoGeneratedRounds, (newVal) => {
  if (newVal && customMaxRounds.value === 40) {
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`Starting simulation with ${params.maxRounds} rounds (custom)`)
  } else {
    params.maxRounds = autoGeneratedRounds.value
    addLog(`Starting simulation with ${params.maxRounds} rounds (auto)`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Existing preparation detected, using cached data')
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ Sim duration: ${data.summary.simulation_hours}h`)
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  ├─ Sim duration: ${res.data.summary.simulation_hours}h`)
        ...
        // 配置尚未生成，开始轮询
        addLog('Config generating, polling for completion...')
        startConfigPolling()
      }
    }
  } catch (err) {
...
onMounted(() => {
  // 自动开始准备流程
  if (props.simulationId) {
    addLog('Step2 env setup initialized')
